...
@pytest.mark.skip(reason="called by tests")
...
async def test_cms_responses_nosub_v16(hass, socket_enabled):
    ...
    async with (
        websockets.connect(
            f"ws://127.0.0.1:9001/{cp_id}",  # this is the charger cp_id ie CP_1_nosub in the cs
        ) as ws2
    ):
