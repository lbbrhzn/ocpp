...
class ChargePoint(cp):
    ...
    def build_session_clear_request(self, profile_id: int):
        """Build an exact-id 2.0.1 clear that preserves foreign profiles."""
        return call.ClearChargingProfile(charging_profile_id=int(profile_id))

    def build_custom_profile_request(self, connector_id: int, profile: dict):
        """Build a custom 2.0.1 profile without changing its supplied content."""
        evse_id = 0
        if connector_id > 0:
            evse_id, _ = self._global_to_pair(int(connector_id))
        return call.SetChargingProfile(evse_id, profile)
