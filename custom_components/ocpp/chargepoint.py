...
# SetChargingProfile answers with the same status word on 1.6 and 2.0.1, so the
# shared session-limit flow compares the word rather than either enum.
SESSION_LIMIT_ACCEPTED = "Accepted"
...
class ChargePoint(cp):
    ...
    async def set_session_limit(self, connector_id: int, limit_amps: float) -> bool:
        ...
        if status == SESSION_LIMIT_ACCEPTED:
            return True
        raise HomeAssistantError(
